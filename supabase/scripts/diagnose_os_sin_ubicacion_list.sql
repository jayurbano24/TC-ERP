-- PASO 2 — Detalle fila a fila (~86 OS). Ejecutar solo este archivo.
-- Si timeout: ejecutar PASO 1 primero (diagnose_os_sin_ubicacion_detail.sql)
...
sa AS (
  SELECT s.service_order_id AS os_id,
    ...
    string_agg(DISTINCT s.serial_number || ':' || s.current_status::text, ' | '
      ORDER BY s.serial_number || ':' || s.current_status::text) AS detalle_series
...
SELECT
  e.os_label AS "OS",
  e.main_serial AS "Serie",
  e.os_status AS "Status OS",
  e.sap_integration_status AS "SAP",
  CASE
    WHEN e.n_series = 0 THEN 'Sin serie en planta'
    WHEN e.in_reac_suelto THEN 'ready_to_dispatch sin caja'
    WHEN e.detalle_series LIKE '%RECEPCIONADO_BODEGA_GENERAL%' AND NOT e.in_pendiente_bodega
      THEN 'RECEPCIONADO BO sin bandeja CAC'
    WHEN e.detalle_series LIKE '%in_central_warehouse%' AND NOT e.in_bodega_caja AND NOT e.in_equipo_listo
      THEN 'in_central_warehouse sin bucket'
    WHEN e.detalle_series LIKE '%irreparable%' THEN 'irreparable sin scrap bucket'
    ELSE 'Huérfano — revisar'
  END AS "Motivo",
  e.detalle_series AS "Series (serial:status)",
  e.n_series AS "N series",
  e.os_id AS "UUID"
FROM eval e
WHERE NOT (
