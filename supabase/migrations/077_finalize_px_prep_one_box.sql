-- 077: Prep PX — una caja por llamada (evita timeout SQL Editor en prep de 27 cajas)
...
BEGIN
  ...
  IF v_box.status::text NOT IN ('cerrada', 'closed') THEN
    RAISE EXCEPTION 'BOX_NOT_CLOSED: Caja % no está cerrada.', v_box.id;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.finalize_px_reception_prep_one_box_tx(uuid, integer)
