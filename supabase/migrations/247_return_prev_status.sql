-- 247 — Guardar PrevStatus en devoluciones (reverso sin enum inválido)
BEGIN;
...
BEGIN
  ...
  SET
    ...
    notes =
      ...
      'PrevStatus: ' || current_status::text || E'\n' ||
      CASE
        WHEN p_observaciones IS NOT NULL AND trim(p_observaciones) <> '' THEN
          'Observaciones: ' || trim(p_observaciones) || E'\n'
        ELSE ''
      END ||
      E'\n' || COALESCE(notes, ''),
    updated_at = v_now
