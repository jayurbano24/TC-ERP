-- =============================================================================
-- 121 — Optimizar next_equipment_reentry_count (timeout en classify CAC)
--
-- Problema: la función de 119 hacía full-scan de service_orders/series con
-- upper(trim(...)) por cada unidad del lote → "statement timeout".
-- =============================================================================

CREATE INDEX IF NOT EXISTS idx_service_orders_main_serial_upper
  ON public.service_orders (upper(main_serial));

CREATE INDEX IF NOT EXISTS idx_series_serial_number_upper
  ON public.series (upper(serial_number));
...
AS $$
  ...
  prior AS (
    ...
    JOIN cleaned c ON upper(so.main_serial) = c.sn

    UNION

    SELECT s.service_order_id AS id
    FROM public.series s
    JOIN cleaned c ON upper(s.serial_number) = c.sn
...
-- classify: subir timeout y reusar la función optimizada (misma firma)
