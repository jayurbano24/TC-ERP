    ...
    <>
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-black text-[var(--heading)] uppercase tracking-tighter">Procesando Guía: <span className="text-[var(--accent)] ml-2">{scannedGuides.map(g => g.split(' ')[0]).join(' / ') || activeReception.guide_number?.split(' ')[0]}</span></h2>
          </div>
          <div className="flex gap-2">
             <Badge className="bg-[var(--heading)] text-white px-6 py-2 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] border-none shadow-lg">Lote ID: {activeReception.id.substring(0,8)}</Badge>
          </div>
        </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl grid grid-cols-1 md:grid-cols-3 gap-10 mb-8 relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[var(--accent)] to-[var(--heading)]"></div>
          ...
          >
            <div className="bg-slate-50 p-5 rounded-2xl text-[var(--heading)] shadow-inner group-hover/btn:bg-[var(--heading)] group-hover/btn:text-white transition-all"><Truck size={28} /></div>
            <div className="flex-1 overflow-hidden">
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Tienda / Agencia Destino</p>
              <h3 className="text-xl font-black text-[var(--heading)] uppercase truncate leading-tight group-hover/btn:text-[var(--accent)] transition-colors">{agencyDetails?.name || 'SELECCIONAR AGENCIA'}</h3>
              <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase flex items-center gap-2">
                <MapPin size={12} className="text-[var(--accent)]" /> {agencyDetails?.direccion || 'SIN DIRECCIÓN REGISTRADA'}
              </p>
            </div>
          </button>
          
          <div className="flex items-start gap-6 border-r border-slate-50 pr-6">
            <div className="bg-blue-50 p-5 rounded-2xl text-[var(--accent)] shadow-inner"><UserCheck size={28} /></div>
            <div>
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Encargado de Tienda</p>
              <h3 className="text-xl font-black text-[var(--heading)] uppercase leading-tight">{agencyDetails?.manager || 'PENDIENTE'}</h3>
              <p className="text-[10px] font-bold text-slate-400 mt-2 flex items-center gap-2">
                <Phone size={12} className="text-[var(--accent)]" /> {(agencyDetails as any)?.telefono || 'SIN TELÉFONO'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-[var(--heading)] uppercase leading-tight">{processingDateLabel || '---'}</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase mt-2">
                Usuario: {currentUserFullName || 'SISTEMA'}
              </p>
            </div>
          </div>
        </div>
