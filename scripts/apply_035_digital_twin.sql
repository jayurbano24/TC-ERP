-- =============================================================================
-- APLICAR MIGRACIÓN 035 — Gemelo digital (Motor 1-4)
-- =============================================================================
-- Ejecutar TODO este archivo en Supabase → SQL Editor (una sola vez).
-- NO ejecutar solo refresh_service_order_operational_states() antes de esto.
--
-- Prerrequisitos: migraciones 033 (cac_tray_units) y 034 ya aplicadas.
--
-- Después de éxito, validar con: scripts/validate_digital_twin_snapshot.sql
-- =============================================================================
...
DECLARE
  ...
  v_tray_active boolean;
  v_tray_excluded text;
  v_status text;
  v_channel text;
BEGIN
  ...
  SELECT t.is_active, t.excluded_reason
  INTO v_tray_active, v_tray_excluded
  FROM public.cac_tray_units t
  WHERE t.service_order_id = p_os_id;

  v_tray_active := coalesce(v_tray_active, false);

  IF v_channel = 'px' AND NOT v_tray_active THEN
    RETURN QUERY SELECT
      'px_operativo'::text, 'PX · pipeline'::text, 'px'::text,
      NULL::text, v_tray_active, v_tray_excluded;
    RETURN;
  END IF;

  IF v_channel = 'cac' AND NOT v_tray_active THEN
    RETURN QUERY SELECT
  ...
  IF v_status IS NULL THEN
    RETURN QUERY SELECT
      ...
      NULL::text,
      v_tray_active,
      coalesce(v_tray_excluded, 'no_series');
    RETURN;
  END IF;
  ...
  IF v_status = 'RECEPCIONADO_BODEGA_GENERAL' THEN
    RETURN QUERY SELECT 'pendiente_ingreso_bodega'::text, 'Pendiente ingreso bodega'::text,
      v_channel, v_status, v_tray_active, v_tray_excluded;
  ...
  ELSIF v_status = 'returned' THEN
    RETURN QUERY SELECT 'devuelto'::text, 'Devuelto'::text,
      v_channel, v_status, v_tray_active, v_tray_excluded;
  ELSIF v_status IN ('in_workshop', 'in_qc', 'in_validation', 'in_control_warehouse') THEN
...
GRANT EXECUTE ON FUNCTION public.refresh_service_order_operational_states() TO service_role;
GRANT EXECUTE ON FUNCTION public.refresh_service_order_stage_summary() TO service_role;
