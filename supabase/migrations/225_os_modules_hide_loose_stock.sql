-- 225: Bodega sin caja → solo pistoleo EN_PROCESO/TMP; sin tarjeta "sin series" en UI.
...
WITH agg AS (
  SELECT
    ...
    -- Solo cajas en pistoleo (EN_PROCESO / TMP-*), no stock suelto.
    count(DISTINCT s.service_order_id) FILTER (
...
SELECT jsonb_build_object(
  ...
  'bodega_sin_caja', 0,
  'pistoleo_en_curso', (SELECT pistoleo_en_curso FROM agg),
...
COMMENT ON FUNCTION public.count_os_inventory_modules() IS
  'ETL liviano. Sin series oculto. Bodega suelta no se reporta; pistoleo_en_curso = TMP/EN_PROCESO.';
