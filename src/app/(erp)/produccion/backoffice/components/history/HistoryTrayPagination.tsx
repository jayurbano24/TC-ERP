...
export function HistoryTrayPagination({ totalCount, safePage, totalPages, startItem, endItem, setHistoryPage }: Props) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-5 border-t border-[var(--border)]">
      <div className="flex items-center gap-6">
        <p className="text-[10px] font-black text-[var(--muted)] uppercase tracking-widest">
          Mostrando <span className="text-[var(--foreground)]">{startItem}-{endItem}</span> de <span className="text-[var(--foreground)]">{totalCount}</span> equipos CAC (TC-XXX)
        </p>
