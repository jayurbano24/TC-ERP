...
type Props = {
  ...
  onClose: () => void;
  onConfirm: () => void;
  /** Si viene de cola SCRAPS, no ofrecer destino SCRAPS. */
  fromScraps?: boolean;
};
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-black text-base sm:text-lg truncate">
                {fromScraps ? 'Regresar desde SCRAPS' : 'Mover Equipo de Etapa'}
              </h3>
              <p className="text-[10px] font-bold text-amber-100 uppercase tracking-widest mt-0.5">
        ...
        <div className="p-4 sm:p-6 space-y-6 bg-slate-50 overflow-y-auto flex-1 min-h-0">
          <p className="text-xs font-bold text-slate-600">
            ...
            {item?.os ? (
              <>
                {' '}
                (OS <span className="font-black text-[#181c3a]">{item.os}</span>)
              </>
            ) : null}
            :
          </p>
          <div>
            ...
            <select value={returnTargetStage} onChange={e => setReturnTargetStage(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-[#181c3a] outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-all">
              ...
              {!fromScraps && <option value="irreparable">SCRAPS</option>}
            </select>
          </div>
          <Button
            variant="primary"
