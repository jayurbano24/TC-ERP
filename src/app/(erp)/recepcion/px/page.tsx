    ...
    >
      <div className="flex items-center gap-4 mb-8 border-b border-slate-100">
        <button 
          ...
          className={`pb-4 px-2 text-sm font-black uppercase tracking-widest transition-all relative ${activeTab === 'scan' ? 'text-[var(--heading)]' : 'text-slate-300 hover:text-slate-400'}`}
        >
          <div className="flex items-center gap-2">
            <Scan className="w-4 h-4" />
            Ingreso de Equipos
          </div>
          {activeTab === 'scan' && <div className="absolute bottom-0 left-0 w-full h-1 bg-[var(--accent)] rounded-t-full" />}
        </button>
        <button 
          onClick={() => setActiveTab('history')}
          className={`pb-4 px-2 text-sm font-black uppercase tracking-widest transition-all relative ${activeTab === 'history' ? 'text-[var(--heading)]' : 'text-slate-300 hover:text-slate-400'}`}
        >
          <div className="flex items-center gap-2">
            <History className="w-4 h-4" />
            Historial de Recepciones
          </div>
          {activeTab === 'history' && <div className="absolute bottom-0 left-0 w-full h-full bg-[var(--accent)]/5 rounded-t-full border-b-4 border-[var(--accent)]" />}
          {activeTab === 'history' && <div className="absolute bottom-0 left-0 w-full h-1 bg-[var(--accent)] rounded-t-full" />}
        </button>
      </div>
      ...
      {activeTab === 'scan' && (
        ...
        <div className="lg:col-span-3 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none focus:border-[var(--accent)]"
                    value={guideData.sap}
                ...
                <Button 
                  variant="outline" 
                  className="w-full border-dashed border-2 border-slate-200 text-slate-500 hover:border-[var(--accent)] hover:text-[var(--accent)] hover:bg-[var(--accent)]/5 font-black text-[10px] uppercase tracking-widest h-12"
                  onClick={() => {
          ...
          <Card className="bg-[var(--heading)] text-white border-none">
            <div className="space-y-6">
                  ...
                  return (
                    <div key={item.id} className="bg-white/5 p-4 rounded-2xl border border-white/10 flex justify-between items-center group transition-all hover:bg-white/10">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[8px] font-medium text-[var(--accent)] uppercase mt-1">{item.tecnologia}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-xl font-black text-[var(--accent)]">{count}</span>
              ...
              <div className="pt-4 border-t border-white/10">
                <Button 
                  variant="primary" 
                  className="w-full bg-[var(--accent)] hover:bg-[#1fb0db] text-[var(--heading)] h-12 shadow-lg shadow-[var(--accent)]/20"
        ...
        <div className="lg:col-span-9 space-y-6">
          <Card className="p-8">
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Scan className="w-6 h-6 text-[var(--heading)]" />
                  <h2 className="text-lg font-black text-[var(--heading)]">Escaneo de Series (SN)</h2>
                </div>
              ...
              <form onSubmit={handleAddSN} className="flex gap-4">
                <div className="relative flex-1 group">
                  <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 group-focus-within:text-[var(--accent)] transition-colors" />
                  <input 
                    type="text" 
                    value={currentSN}
                    onChange={(e) => setCurrentSN(e.target.value)}
                    placeholder="Escanee serie del equipo..."
                    className="w-full h-16 pl-14 pr-6 bg-slate-50 border-2 border-slate-100 rounded-2xl text-xl font-mono font-bold outline-none focus:border-[var(--accent)] focus:bg-white transition-all"
                  />
                </div>
                <Button type="submit" className="h-16 px-10 rounded-2xl shadow-xl shadow-[var(--heading)]/10">
                ...
                <tbody className="divide-y divide-slate-50">
                  {manifestItems.map((item) => {
                    ...
                    return (
                      <tr 
                        key={item.id} 
                        className={`font-bold cursor-pointer transition-colors ${selectedModelForScan === item.modelo ? 'bg-[var(--accent)]/5' : ''} ${isComplete ? 'bg-emerald-50/20' : 'hover:bg-slate-50'}`}
                        onClick={() => setSelectedModelForScan(item.modelo)}
                      >
                        <td className="px-6 py-4">
                          <span className="text-[var(--accent)] font-black text-[10px] uppercase tracking-tighter">{item.tecnologia}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                        ...
                        <td className="px-6 py-4 text-center text-sm font-black text-[var(--heading)]">
                          {scannedForThis}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-4">
                            <div className="flex flex-col items-end gap-1.5 flex-1">
                              <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                  className={`h-full transition-all duration-500 ${isComplete ? 'bg-emerald-500' : 'bg-[var(--accent)]'}`}
          ...
          <Card padding="none" className="overflow-hidden border-none shadow-xl">
            <div className="bg-[var(--heading)] p-4 flex justify-between items-center">
              ...
              <span className="text-[10px] font-black text-[var(--accent)] bg-[var(--accent)]/10 px-2 py-1 rounded">MODO: INGRESO DIRECTO</span>
            </div>
                    ...
                    <tr key={sn.sn} className="hover:bg-slate-50 transition-colors animate-rise-in border-b border-slate-50">
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button 
                            className="p-1 text-slate-400 hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 rounded-md transition-all"
                            title="Editar"
                          >
      ...
      {activeTab === 'history' && (
        <div className="space-y-6 animate-rise-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="bg-[var(--heading)] p-2 rounded-xl">
                <History className="w-5 h-5 text-[var(--accent)]" />
              </div>
              <div>
                <h2 className="text-xl font-black text-[var(--heading)]">Historial de Recepciones PX</h2>
          ...
          <Card padding="none" className="overflow-hidden border-none shadow-xl">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-[var(--heading)] text-white/40 text-[10px] font-black uppercase tracking-widest">
              ...
              <tbody className="divide-y divide-slate-50">
                {[
                  ...
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-mono font-black text-[var(--heading)]">{row.sap}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button className="p-2 text-slate-400 hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 rounded-xl transition-all">
