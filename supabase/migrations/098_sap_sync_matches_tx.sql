-- 098: sync SAP compacto — solo coincidencias + reset set-based de no match.
-- Evita enviar 15k–25k filas de "Sin Coincidencia" en el body HTTP.
...
BEGIN
  ...
  -- Detalles solo de coincidencias (payload chico)
  ...
  FROM jsonb_to_recordset(p_matched_series) AS m(id uuid)
  WHERE s.id = m.id;
  ...
  IF p_reset_unmatched THEN
    -- Una sola sentencia SQL: no requiere listar IDs desde el cliente
    UPDATE public.series s
