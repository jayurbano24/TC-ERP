-- 124 Fix next_box_code: LPAD(2) truncaba BOX-100+ a BOX-10
-- En Postgres, LPAD('1000', 2, '0') = '10' (trunca), no '1000'.
-- Con cajas hasta BOX-999, el correlativo siguiente siempre colisionaba.
...
BEGIN
  ...
  WHERE s.schemaname = 'public'
    AND s.sequencename = 'box_code_seq';
  ...
  LOOP
    ...
    -- NO usar LPAD(..., 2): trunca valores >= 100
    IF v_next < 10 THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE b.box_code = v_code
        AND upper(trim(coalesce(b.rack_location, ''))) NOT IN ('ELIMINADO', 'DESPACHO')
    ) THEN
